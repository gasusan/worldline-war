...
function makeDeck(f){
  ...
  const d=[]; while(d.length<30) d.push(names[d.length%names.length]);
  return shuffle(d);
}
...
function applyTrap(room,owner,enemyOwner,lane,pos,u){
  ...
  const t=traps.shift(); const c=CARDS[t.name];
...
function action(room,pidx,msg){
  ...
  if(a==="resource"){
    ...
    if(c.type==="unit"){
      ...
      p.facility={name}; log(room,`🏭 ${name} を設置`);
    } else if(c.type==="trap"){
      const pos=msg.pos??2; p.traps[pos]=p.traps[pos]||[]; p.traps[pos].push({name}); log(room,`💣 ${name} を${pos+1}マス目に設置`);
    } else {
      if(c.effects?.includes("draw2")) draw(p,2);
        ...
        if(np<0||np>6) {room.players[1-pidx].base-=1; p.board[target.pos]=p.board[target.pos].filter(x=>x.id!==target.id); p.grave.push(target.name); draw(p,1); draw(room.players[1-pidx],2); log(room,`🏰 総動員令：${target.name}が本拠地へ1ダメージ`);}
        else {const def=room.players[1-pidx].board[np].find(x=>x.lane===target.lane); if(def) resolveBattle(room,pidx,target,np,target.lane,def); else {p.board[target.pos]=p.board[target.pos].filter(x=>x.id!==target.id);p.board[np].push(target);target.pos=np;applyTrap(room,pidx,1-pidx,target.lane,np,target);} }
      }
      else if(c.effects?.includes("reset")){p.grave.forEach(n=>p.deck.push(n));p.grave=[];shuffle(p.deck);}
      else if(c.effects?.includes("parliament")){ const limit=unitsCount(room); const actual=Math.max(0,12-limit); if(actual>0){}; for(let oi=0;oi<2;oi++){const op=room.players[1-pidx];op.board=op.board.map(row=>row.filter(u=>{if(CARDS[u.name].cost<=3){op.hand.push(u.name);return false}return true;}));}}
      else if(c.effects?.includes("tradeDiscard")){ if(room.players[1-pidx].hand.length) p.grave.push(p.hand.pop()); }
      else if(c.effects?.includes("memory")){ for(let i=0;i<Math.min(7,p.grave.filter(n=>n==="独りよがりな骸").length);i++) if(p.deck.length)p.resources.push({name:p.deck.pop(),used:false});}
      else if(c.effects?.includes("reappear")){ const lane=msg.lane??0; const idx=p.grave.indexOf("独りよがりな骸"); if(idx>=0){p.grave.splice(idx,1);addUnit(p,"独りよがりな骸",lane,0);} else {for(let i=0;i<2&&p.deck.length;i++)p.grave.push(p.deck.pop());}}
      log(room,`📜 ${name} を使用`);
    }
  } else if(a==="move"){
...
server.listen(PORT,()=>console.log(`WORLDLINE WAR listening on ${PORT}`));
